package com.micen.buyers.widget.flowlayout;
...
public class FlowLayout extends ViewGroup
{
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		this.setMeasuredDimension(resolveSize(totalControlWidth, widthMeasureSpec),
	...
	private void applyPositionsToViews(LineDefinition line)
	{
		for (View child : line.getViews())
		{
			...
			if (this.config.getOrientation() == HORIZONTAL)
			{
				...
				child.measure(MeasureSpec.makeMeasureSpec(layoutParams.getLength(), MeasureSpec.EXACTLY),
			...
			else
			{
				...
				child.measure(MeasureSpec.makeMeasureSpec(layoutParams.getThickness(), MeasureSpec.EXACTLY),
	...
	private void applyGravityToLine(LineDefinition line)
	{
		...
		int excessLength = line.getLineLength()
				- (lastChildLayoutParams.getLength() + lastChildLayoutParams.getInlineStartLength());
		int excessOffset = 0;
		for (View child : line.getViews())
		{
			LayoutParams layoutParams = (LayoutParams) child.getLayoutParams();
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			child.layout(lp.x + lp.leftMargin, lp.y + lp.topMargin, lp.x + lp.leftMargin + child.getMeasuredWidth(),
					lp.y + lp.topMargin + child.getMeasuredHeight());
		}
	}

...
public class LinkedIn
{
			...
			final LinkedInDialogListener listener)
	{
		...
		dialog(ctx, handler, new LinkedInDialogListener()
		{
			...
			public void onComplete(Bundle values)
			{
				...
				edit.putString(OAuth.OAUTH_TOKEN, values.getString(ACCESS_TOKEN));
				edit.putString(OAuth.OAUTH_TOKEN_SECRET, values.getString(SECRET_TOKEN));
				edit.commit();
				if (isSessionValid())
				{
	...
	public void dialog(final Context ctx, Handler handler, final LinkedInDialogListener listener)
	{
		...
		new LnDialog(ctx, mHttpOauthProvider, mHttpOauthConsumer, listener, mIcon).show();
	}
	...
	public void shareMessage(String resultXML,String consumerKey,String consumerSecret) throws Exception
	{
		...
		post.addHeader("User-Agent", System.getProperties().getProperty("http.agent") + " LinkedInAndroidSDK");
